...
export const Hero = ({ onScroll, brandRef }: HeroProps) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Update the transform calculation to trigger faster when approaching the launching soon section
      const scrollThreshold = window.innerHeight / 3; // Reduced threshold for faster trigger
      const scrollProgress = Math.min(1, window.scrollY / scrollThreshold);
      const scale = scrollProgress > 0.5 ? 0.15 : 1 - (scrollProgress * 1.7); // Faster scaling
  ...
  return (
    ...
    >
      ...
      <div className="fixed z-[60] px-4" style={{
        ...
        <h1 
          ref={brandRef}
          className="text-4xl sm:text-5xl md:text-7xl lg:text-9xl text-white tracking-[0.25rem] md:tracking-[0.5rem] font-serif-display transition-transform duration-300"
          style={{
